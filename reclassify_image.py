...
def predict_zarr(zarr_path, model, bands, chunk_size):
    ...
    output_fname = output_path.replace('.nc', '.tif')
    if os.path.exists(output_fname):
        logging.info(f'Predictions already exist for {zarr_path}. Skipping...')
        return
    ...
    for lat_start in range(0, ds.sizes['lat'], chunk_size):
        for lon_start in range(0, ds.sizes['lon'], chunk_size):
            ...
            data = pd.DataFrame(data, columns=[f'{b}_{m}' for b in bands for m in range(1, 13)])
    ...
    pred_da.rio.to_raster(output_path.replace('.nc', '.tif'), driver="GTiff", compress="LZW", tiled=True)
...
if __name__ == '__main__':
    ...
    for year in range(2021, 2022):
        ...
        model_path = f'{parent_dir}/data/xgb_{year}_model_winter_wheat.json'
